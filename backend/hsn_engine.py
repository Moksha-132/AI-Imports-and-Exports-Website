...
class HSNEngine: 
    ...
    def _load_database(self) -> List[Dict[str, Any]]:
        try:
            if not os.path.exists(HSN_DATABASE_PATH): 
                return []
            with open(HSN_DATABASE_PATH, 'r', encoding='utf-8') as f: 
                return json.load(f)
        except: 
            return []
    ...
    def _generate_explanation(self, query: str, match: Dict[str, Any], score: float) -> str:
        ...
        if score > 0.8: 
            return f"High confidence match: {', '.join(list(hits)[:3])}."
...
def get_hsn(text: str) -> Dict[str, Any]: 
    return _hsn_engine_instance.classify_product(text)
def lookup_hsn(code: str) -> Optional[Dict[str, Any]]: 
    return _hsn_engine_instance.lookup_code(code)