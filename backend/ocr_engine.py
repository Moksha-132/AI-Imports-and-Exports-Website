...
def extract_text_from_binary(file_bytes: bytes, filename: str) -> str:
    ...
    try:
        ...
        logger.error(f"Text extract fail: {e}")
    return txt.strip()

def parse_extracted_text(text: str) -> Dict[str, Any]:
    res = {"invoice_no": None, "vendor": None, "amount": 0.0, "currency": "USD", "hsn_codes": [], "items": [], "destination": "United States", "client_name": "Primary Client", "transport_mode": "Sea"}
    ...
    for p in inv_p:
        ...
        if m: 
            res["invoice_no"] = m.group(1).upper(); 
            break
    ...
    for c in countries:
        if re.search(rf'\b{c}\b', area, re.I): 
            found.append(c)
    if found: 
        res["destination"] = found[-1]
    else:
        for c in reversed(countries):
            if re.search(rf'\b{c}\b', text, re.I): 
                res["destination"] = c; 
                break
    if re.search(r'\b(Air|Flight|AWB|Aviation|Airport|Aero)\b', text, re.I):
        res["transport_mode"] = "Air"
    elif re.search(r'\b(Road|Land|Truck|Trucking|Trailer|Highway)\b', text, re.I):
        res["transport_mode"] = "Land"
    elif re.search(r'\b(Sea|Ocean|Vessel|Port|Container|B/L|Marine)\b', text, re.I):
        res["transport_mode"] = "Sea"
...
def create_humanized_summary(data: Dict[str, Any]) -> str:
    ...
    if data['amount'] > 0: 
        m += f" Value: {data['currency']} {data['amount']:,.2f}."
    return m
    
def process_document(file_bytes: bytes, filename: str) -> Dict[str, Any]:
    txt = extract_text_from_binary(file_bytes, filename)
    if not txt: 
        return {"invoice_no": None, "vendor": "Unknown", "amount": 0, "currency": "USD", "hsn_codes": [], "items": [], "humanized_summary": "Unreadable."}
