    ...
    <div className="space-y-12 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-8 bg-white p-10 rounded-[2.5rem] border border-slate-100">
          ...
          <div className="h-72 flex items-end gap-3 border-b border-slate-100">
            {(tradeStats.monthly_breakdown?.length === 12 ? tradeStats.monthly_breakdown : [40, 65, 45, 80, 55, 90, 70, 85, 60, 75, 50, 95]).map((v, i) => {
              const max = Math.max(...(tradeStats.monthly_breakdown?.length === 12 ? tradeStats.monthly_breakdown : [100]), 1);
              const h = (v / max) * 100;
              const isCurrentMonth = i === new Date().getMonth();
              const hasRealData = tradeStats.monthly_breakdown?.length === 12;

              return (
                <div key={i} className="flex-1 flex flex-col items-center group relative h-full justify-end">
                  <div className="absolute -top-10 left-1/2 -translate-x-1/2 px-3 py-1 bg-slate-900 text-white text-[10px] font-bold rounded opacity-0 group-hover:opacity-100 transition-opacity z-20 whitespace-nowrap">
                    {hasRealData ? `$${v.toLocaleString()}` : `${v}%`}
                  </div>
                  <div
                    style={{ height: `${Math.max(4, h)}%` }}
                    className={`w-full max-w-[24px] rounded-t-sm transition-all duration-300 ${isCurrentMonth ? "bg-amber-500" : (hasRealData && v > 0) ? "bg-slate-800" : "bg-slate-100 group-hover:bg-slate-200"
                      }`}
                  />
                  <span className="absolute -bottom-8 text-[10px] font-medium text-slate-400">{getMonthName(i)}</span>
                </div>
              );
            })}
          </div>
